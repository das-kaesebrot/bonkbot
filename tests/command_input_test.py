...
def patch_bot_side_effects(bonk_bot: BonkBot) -> Generator[None, None, None]:
    ...
    with patch.object(bonk_bot, "_is_admin", AsyncMock(return_value=True)):
        with patch.object(
            bonk_bot, "_send_to_horny_jail", AsyncMock(side_effect=_mock_send_to_jail)
        ):
...
class TestUserResolutionInputs:
    def test_reply_resolved_returns_author(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
    ...
    def test_reply_unresolved_fetches_and_returns_author(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
        referenced_author = MagicMock(spec=discord.User)
    ...
    def test_mention_returns_first_mention(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
    ...
    def test_no_args_returns_none(
        self, bonk_bot: BonkBot, mock_message: MagicMock, search_arg: str | None
    ) -> None:
    ...
    def test_nick_match_lowercase(
        self, bonk_bot: BonkBot, mock_message: MagicMock, mock_member: MagicMock
    ) -> None:
    ...
    def test_no_match_returns_none(
        self, bonk_bot: BonkBot, mock_message: MagicMock, search_arg: str
    ) -> None:
        result = _run(bonk_bot._get_user_from_message(mock_message, search_arg))
        assert result is None

    def test_reply_preferred_over_mention(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
        referenced_author = MagicMock(spec=discord.User)
...
class TestBonkPrefixInputs:
    def test_no_args_returns_info(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
        result = _run(
            bonk_bot._handle_command(
    ...
    def test_empty_or_blank_args_returns_info(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        cached_guild: Guild,
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.PREFIX, arg, cached_guild)
        )
        assert result == BotMessage.GUILD_PREFIX_INFO.format("!")

    @pytest.mark.parametrize("char", ["?", ".", "$", "#", "@", "~"])
    def test_valid_single_char_sets_prefix(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild, char: str
    ) -> None:
        result = _run(
            bonk_bot._handle_command(
...
class TestBonksInputs:
    def test_no_args_returns_leaderboard(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.BONKS, None, cached_guild)
        )
        assert result == "**TOP BONKS**\n"

    @pytest.mark.parametrize("arg", ["self", "testmember", "member", "test"])
    def test_user_resolution_returns_bonk_count(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        mock_member: MagicMock,
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.BONKS, arg, cached_guild)
        )
        assert result == BotMessage.USER_BONKS_INFO.format(
            name=mock_member.display_name, amount=0
        )

    def test_with_mention_returns_bonk_count(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
        mentioned = MagicMock(spec=discord.Member)
        mentioned.id = 777
    ...
    def test_no_match_returns_error(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild, arg: str
    ) -> None:
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.BONKS, arg, cached_guild)
        )
...
class TestBonkInputs:
    def test_no_args_returns_error(
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.BONK, None, cached_guild)
        )
        assert result == BotError.NO_USER_FOUND.format(None)

    @pytest.mark.parametrize("arg", ["self", "testmember", "member", "test"])
    def test_user_resolution_bonks_user(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        mock_member: MagicMock,
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.BONK, arg, cached_guild)
        )
        assert result == BotMessage.BONK.format(name=mock_member.display_name, amount=1)

    def test_with_mention_bonks_user(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
        mentioned = MagicMock(spec=discord.Member)
        mentioned.id = 888
        ...
        assert result == BotMessage.BONK.format(name=mentioned.display_name, amount=1)
    ...
    def test_reply_preferred_over_args(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
        referenced_author = MagicMock(spec=discord.User)
...
class TestBonkPardonInputs:
    ...
    @pytest.mark.parametrize("arg", ["self", "testmember", "member", "test"])
    def test_non_jailed_user_returns_none(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild, arg: str
    ) -> None:
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.PARDON, arg, cached_guild)
        )
        assert result is None
...
class TestHornyJailInputs:
    def test_no_args_returns_error(
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.JAIL, None, cached_guild)
        )
        assert result == BotError.NO_USER_FOUND.format(None)

    @pytest.mark.parametrize("arg", ["self", "testmember", "member", "test"])
    def test_user_resolution_jails_user(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        mock_member: MagicMock,
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.JAIL, arg, cached_guild)
        )
...
class TestAdminRoleInputs:
    ...
    def test_no_or_empty_args_returns_info(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        cached_guild: Guild,
    ...
    def test_one_role_mention_sets_role(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
    ...
    def test_zero_role_mentions_returns_error(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
    ...
    def test_multiple_role_mentions_returns_error(
        self, bonk_bot: BonkBot, mock_message: MagicMock, cached_guild: Guild
    ) -> None:
        mock_message.role_mentions = [
...
class TestJailTimeInputs:
    ...
    def test_no_or_empty_args_returns_info(
        ...
        assert result == BotMessage.JAIL_TIME_INFO.format(
            cached_guild.horny_jail_seconds
        )

    @pytest.mark.parametrize(
        "arg,expected", [("5", 5), ("0", 0), ("1", 1), ("9", 9), ("600", 600)]
    )
    def test_valid_integer_sets_time(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        cached_guild: Guild,
...
class TestJailAmountInputs:
    def test_no_args_returns_info(
        ...
        assert result == BotMessage.JAIL_BONKS_INFO.format(
            cached_guild.horny_jail_bonks
        )

    @pytest.mark.parametrize(
        "arg,expected", [("5", 5), ("0", 0), ("1", 1), ("9", 9), ("600", 600)]
    )
    def test_valid_integer_sets_amount(
        self,
        bonk_bot: BonkBot,
        mock_message: MagicMock,
        cached_guild: Guild,
...
class TestHelpInput:
    ...
    def test_returns_help_message(
        ...
        result = _run(
            bonk_bot._handle_command(mock_message, BotCommand.HELP, arg, cached_guild)
        )
...
class TestOnMessageRouting:
    def test_message_with_prefix_parses_command(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
    ...
    def test_message_with_prefix_and_args(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
        mock_message.content = "!bonkprefix ?"
        with patch.object(bonk_bot, "_handle_command", AsyncMock(return_value=None)):
    ...
    @pytest.mark.parametrize(
        "content", ["hello world", "foo bar", "123", "test message"]
    )
    def test_message_without_prefix_ignored(
        self, bonk_bot: BonkBot, mock_message: MagicMock, content: str
    ) -> None:
    ...
    def test_bare_bonk_allowed_without_prefix(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
        mock_message.content = "bonk"
        cached_prefix = bonk_bot._BonkBot__data_service.get_guild_prefix(
            mock_message.guild.id
        )
    ...
    def test_bonk_with_args_allowed_without_prefix(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
    ...
    def test_self_messages_ignored(
        self, bonk_bot: BonkBot, mock_message: MagicMock, content: str
    ) -> None:
        mock_message.content = content
    ...
    def test_prefix_stripped_before_parsing(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
    ...
    def test_multiple_args_joined_correctly(
        self, bonk_bot: BonkBot, mock_message: MagicMock
    ) -> None:
    ...
    @pytest.mark.parametrize(
        "content", ["bonkhelp", "bonkxyz", "bonkfoo", "bonk123", "BONKhelp"]
    )
    def test_non_bonk_messages_without_prefix_ignored(
        self, bonk_bot: BonkBot, mock_message: MagicMock, content: str
    ) -> None:
        mock_message.content = content
        with patch.object(bonk_bot, "_handle_command", AsyncMock()) as mock:
